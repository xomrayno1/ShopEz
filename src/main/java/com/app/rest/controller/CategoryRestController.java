...
public class CategoryRestController {
	...
	public List<Category> findAll(){
		...
		List<Category> categorieActives = new ArrayList<>();
		for(Category category : categories) {
			if (category.getStatus() == Constant.Status.ACTIVE.getValue() ) {
				categorieActives.add(category);
			}
		}
		return categorieActives;
	}
	...
	public ResponseEntity<APIResponse> getListPagingSortSearchFilter(@RequestBody CategoryPagingSearchSortModel cpssm){
		Page<Category> categories =  categoryService.doFilterSearchPagingCategory(cpssm.getSearchKey(), cpssm.getPageSize(), cpssm.getPageNumber());
		try {
			if(categories == null) {
	...
	public ResponseEntity<APIResponse> deleteCategory(@RequestBody DeleteRequest deleteRequest){
		try {
	...
	public ResponseEntity<APIResponse> updateCategory(@Validated @RequestBody UpdateCategoryRequest categoryRequest){
		...
		if(categoryById != null) {
			...
			try {
				...
				Category category = mapper.map(categoryRequest, Category.class);
				category.setStatus(categoryById.getStatus());
